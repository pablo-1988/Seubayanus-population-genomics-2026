#!/bin/bash
# =============================================================================
# MUM&Co v3.8 — Structural Variant Detection by Whole-Genome Alignment
# =============================================================================
# Description:
#   Detects structural variants (SVs) between two genome assemblies using
#   whole-genome alignment with MUMmer4 (nucmer). MUM&Co identifies deletions,
#   insertions, inversions, translocations, contractions, duplications, and
#   mobile element insertions by comparing alignments between a reference and
#   a query genome. This script was used to identify SVs in S. eubayanus strains
#   relative to the reference strain CBS12357.
#
# Example strain: UCD646 (Holarctic-admixed S. eubayanus, sequenced by Oxford Nanopore)
#   Usage: bash mumandco_v3.8.sh -r CBS12357.fa -q UCD646.assembly.final.fa \
#              -g 12500000 -o CBS_UCD646 -t 8
#
# Key Parameters:
#   -r / --reference_genome : Reference genome FASTA (CBS12357 polished assembly)
#   -q / --query_assembly   : Query genome FASTA (strain to compare against reference)
#   -g / --genome_size      : Estimated genome size in bp (e.g., 12500000 for S. eubayanus)
#   -o / --output_prefix    : Prefix for all output files
#   -t / --threads          : Number of threads for nucmer alignment
#   -m / --minlength        : Minimum SV length to report (default: 50 bp)
#
# Input:
#   - Reference genome FASTA (e.g., CBS12357.fa)
#   - Query genome FASTA (e.g., UCD646.assembly.final.fa)
#
# Output:
#   - {prefix}.SVs_all.tsv          : All detected SVs with coordinates and type
#   - {prefix}.SVs_all.withfragment.tsv : SVs with inserted/deleted sequence
#   - {prefix}.SVs_all.vcf          : SVs in VCF format
#   - {prefix}.summary.txt          : Summary statistics of SV counts by type
#   - {prefix}_alignments/          : NUCmer alignment files (DELTA, COORDS)
#
# Dependencies:
#   MUMmer4 (nucmer, delta-filter, show-coords), bash, awk, sort
#   Install: conda install -c bioconda mummer4
#
# Reference:
#   Mérot et al. (2021) MUM&Co: accurate detection of all SV types.
#   Bioinformatics. https://doi.org/10.1093/bioinformatics/btab139
# =============================================================================
