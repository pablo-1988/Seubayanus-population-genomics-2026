#!/bin/bash
# =============================================================================
# MUM&Co Non-Redundant Population Dataset Generation
# =============================================================================
# Description:
#   Creates a non-redundant structural variant (SV) dataset from multiple
#   genome assemblies per strain by validating SVs across replicate assemblies.
#   This pipeline was developed by O'Donnell et al. (2022) and adapted for
#   S. eubayanus population-level SV analysis.
#
#   Workflow:
#     Step 1: Run MUM&Co v3.8 for each genome assembly against the reference
#     Step 2: Overlap and cluster raw SV calls across assemblies per strain
#     Step 3: Apply overlap-based filters to remove false positives
#             (SVs must appear in ≥2 assemblies per strain for validation)
#     Step 4: Merge per-strain validated SVs into a population-level dataset
#     Step 5: Apply cross-population overlap filter for final non-redundant set
#
# Input:
#   - genomes/          : directory with FASTA assemblies (naming: strainID.fa)
#   - SGDref.nuclear_genome.tidy.fa : reference genome (S. cerevisiae S288C)
#   - mumandco_v3.8.sh  : MUM&Co script (must be in the same directory)
#
# Output:
#   - MAC_output/       : per-strain raw MUM&Co SV calls
#   - MAC_overlap/      : validated, non-redundant SV calls per strain
#   - Final merged population SV dataset
#
# Dependencies:
#   MUMmer4 (nucmer, delta-filter, show-coords), bash, awk, sort, bedtools
#
# Reference:
#   O'Donnell et al. (2022). "A non-redundant approach to..." [adapted]
#
# Example strain: UCD646 (Holarctic-admixed S. eubayanus, Oxford Nanopore)
# =============================================================================

## This is the workflow used to merge datasets and create a non-redundant SV dataset
